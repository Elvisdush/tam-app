# TAM App Docker Deployment Script (PowerShell) - Fixed Version
...
function New-SSLCertificates {
    ...
    if (-not (Test-Path $certPath) -or -not (Test-Path $keyPath)) {
        try {
            ...
            $cert = New-SelfSignedCertificate -DnsName "localhost" -CertStoreLocation "cert:\CurrentUser\My" -KeyUsage DigitalSignature,KeyEncipherment -KeyLength 2048 -NotAfter (Get-Date).AddYears(1)
            ...
            [System.IO.File]::WriteAllBytes($keyPath, $certBytes)
            
            Write-Success "SSL certificates generated"
        } catch {
            Write-Warning "Failed to generate SSL certificates with PowerShell. Using OpenSSL fallback..."
...
function Test-ServiceHealth {
    ...
    try {
        # Backend health
        $response = Invoke-WebRequest -Uri "http://localhost/api/health" -UseBasicParsing -TimeoutSec 10 -ErrorAction Stop
    ...
    try {
        ...
        $response = Invoke-WebRequest -Uri "http://localhost/secure-api/health" -UseBasicParsing -TimeoutSec 10 -ErrorAction Stop
        Write-Success "Secure backend health check passed"
    } catch {
        Write-Warning "Secure backend health check failed"
    }
}
...
switch ($command.ToLower()) {
    ...
    "help" {
        Write-Host "Usage: .\docker-deploy-fixed.ps1 [command]" -ForegroundColor Cyan
        Write-Host ""
        ...
        Write-Host "  cleanup   - Clean up containers and images" -ForegroundColor White
    ...
    default {
        ...
        Write-Host "Use '.\docker-deploy-fixed.ps1 help' for available commands" -ForegroundColor Yellow
    }
}
