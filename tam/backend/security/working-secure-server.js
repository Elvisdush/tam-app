/**
 * Working Secure API Server
 * Fixed version with proper server binding
 */
...
if (cluster.isMaster) {
  console.log('🔒 Starting TAM App Secure API Server (Working Version)');
  ...
  try {
    ...
    server.on('error', (err) => {
      if (err && err.code === 'EADDRINUSE') {
        ...
        console.error(`   • Set PORT environment variable: $env:PORT=3014&& npm run server:secure\n`);
      } else {
        console.error('❌ Server error:', err);
      }
    });
