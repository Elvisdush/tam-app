...
exports.getTwilioEnv = getTwilioEnv;
exports.sendSignInOtpViaTwilioServer = sendSignInOtpViaTwilioServer;
const TWILIO_MESSAGES = (accountSid) => `https://api.twilio.com/2010-04-01/Accounts/${accountSid}/Messages.json`;
...
function getTwilioEnv() {
    const accountSid = process.env.TWILIO_ACCOUNT_SID?.trim() || process.env.EXPO_PUBLIC_TWILIO_ACCOUNT_SID?.trim();
    const authToken = process.env.TWILIO_AUTH_TOKEN?.trim() || process.env.EXPO_PUBLIC_TWILIO_AUTH_TOKEN?.trim();
    const from = process.env.TWILIO_FROM?.trim() || process.env.EXPO_PUBLIC_TWILIO_FROM?.trim();
    if (!accountSid || !authToken || !from)
        return null;
    return { accountSid, authToken, from };
}
async function sendSignInOtpViaTwilioServer(toE164, code) {
